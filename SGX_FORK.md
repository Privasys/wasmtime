...
This document explains every change made in the Privasys wasmtime fork to
support running wasmtime inside Intel SGX enclaves via the
[Teaclave SGX SDK](https://github.com/apache/incubator-teaclave-sgx-sdk).

The fork is maintained as a **single squashed commit rebased on top of the
upstream release tag** — never merged. The `sgx` branch is therefore always
exactly:

```
<upstream release tag>  ──►  feat(sgx): Teaclave/SGX enclave port
```

- **Upstream base:** `v48.0.2`
- **Release tag:** `privasys-v0.3.0` (consumed by `enclave-os-mini` via
  `tag = "privasys-v0.3.0"`)

All changes are gated behind `#[cfg(target_vendor = "teaclave")]` so that
non-SGX builds are completely unaffected. The port touches 13 files
(9 in `crates/wasmtime`, 2 in `crates/fiber`, plus this doc and a `README.md` note).
...
## Change group 1 — Route Teaclave target to `sys::custom`
...
The `cfg_select!` chain that conditionally exposes `pub mod unix` (Unix-specific
...
The core routing change. The `cfg_select!` chain that selects the system
backend now matches Teaclave **before** `unix` (important: `cfg(unix)` is
true under Teaclave's `sgx_tstd`, so the Teaclave arm has to win first):
...
## Change group 2 — De-activate incompatible subsystems
...
### 5. `crates/wasmtime/src/config.rs` — No `avxvnni` host detection

`detect_host_feature` asks `std::is_x86_feature_detected!("avxvnni")`
(added upstream in v48). Teaclave's `is_x86_feature_detected!` reads the
Intel SGX SDK's fixed CPU-feature bitmask, which has no AVX-VNNI bit, so
the macro rejects the name at compile time. The arm is compiled out for
Teaclave. Nothing is lost: host-feature detection only feeds the ISA-flags
check, which this fork already skips in SGX (see 1. above).

---

## Change group 3 — Fibers (`async` feature)

Enclave OS runs each guest call on a wasmtime fiber so that an async host
function (e.g. an outbound fetch) can suspend the guest mid-call.

### `crates/fiber/src/lib.rs` — Use the `nostd` backend

`cfg(unix)` is true under Teaclave, so the backend selection would pick
the Unix backend (`mmap`/`mprotect` guard pages). A Teaclave arm placed
before `unix` selects the heap-backed `nostd` backend, which switches
stacks with the same x86_64 assembly as Unix:
...
`nostd` stacks have **no guard page**. The embedder must size
`Config::async_stack_size` generously above `Config::max_wasm_stack`.

### `crates/fiber/Cargo.toml` — No `rustix` on Teaclave

Same reason as for `crates/wasmtime`: no system calls inside an enclave.
...
| `config.rs` | 2 | No `avxvnni` host detection | Not in the SGX SDK feature bitmask |
| `fiber/src/lib.rs` | 3 | Route Teaclave → `nostd` fibers | No `mmap`/`mprotect` in SGX |
| `fiber/Cargo.toml` | 3 | Exclude `rustix` | No syscalls in SGX |
...
The fork is deliberately kept as **one commit on top of an upstream release
tag**, never a merge. To move to a newer upstream wasmtime release:

```bash
git remote add upstream https://github.com/bytecodealliance/wasmtime.git
git fetch upstream tag vXX.Y.Z --no-tags

# Replay the single SGX commit onto the new release tag.
git checkout -B sgx vXX.Y.Z
git cherry-pick -x <previous sgx tip>     # resolve conflicts in the 11 gated files

# Sign, then force-push the rebased branch.
git commit --amend -S            # keep it GPG-signed
git push --force-with-lease origin sgx

# Cut the next Privasys release tag and push it.
git tag -s privasys-vA.B.C -m "wasmtime fork rebased onto upstream vXX.Y.Z"
git push origin privasys-vA.B.C
```

Then bump `enclave-os-mini` to the new tag (`tag = "privasys-vA.B.C"` in the
three `wasmtime = { git = ... }` entries), regenerate the lockfiles, rebuild
the enclave, and re-pin the enclave's MRENCLAVE. Because this doc and the
`README.md` note are not compiled into any crate, editing them does not
change the enclave MRENCLAVE.
