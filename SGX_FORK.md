...
- **Upstream base:** `v47.0.1`
- **Release tag:** `privasys-v0.2.0` (consumed by `enclave-os-mini` via
  `tag = "privasys-v0.2.0"`)

All changes are gated behind `#[cfg(target_vendor = "teaclave")]` so that
non-SGX builds are completely unaffected. The port touches 10 files
(8 in `crates/wasmtime`, plus this doc and a `README.md` note).
...
git cherry-pick -x <previous sgx tip>     # resolve conflicts in the 8 gated files
